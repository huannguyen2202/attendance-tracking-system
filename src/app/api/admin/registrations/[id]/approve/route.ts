...
function adminOnly(req: NextRequest) {
  ...
  const p = (verifyAccessToken(a) || verifyRefreshToken(r)) as { sub?: string; role?: string } | null;
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // ts-ignore
    user.approvedBy = admin.sub;
    await user.save();

    return NextResponse.json({ status: 200, message: "Đã duyệt tài khoản." });
